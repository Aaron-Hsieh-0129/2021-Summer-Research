...
void Output::printInit(vvmArray & myArray) {
	...
	for (int k = 0; k <= nz-1;k++){
		z = (double) (k - 0.5) * dz ;
		std::cout << fixed << setprecision(2) << z << "    " << myArray.tb[k] << "    " << myArray.rhou[k] << "     " 
...
void Output::output_pi(int n, vvmArray & myArray) {
	std::fstream foutzeta;
	std::string zetaName = "../outputs/pi/pi_" + std::to_string(n) + ".txt";
